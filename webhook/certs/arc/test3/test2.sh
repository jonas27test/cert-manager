#!/bin/bash
#https://kubernetes.github.io/ingress-nginx/deploy/validating-webhook/#self-signed-certificate
...
echo "hello"
kubectl create secret generic webhook.svc --from-file=key.pem=${TEMP_DIRECTORY}/server-key.pem --from-file=cert.pem=${TEMP_DIRECTORY}/server-cert.pem  -n ${NAMESPACE}
# kubectl create secret generic  webhook.webhook.svc --from-file=key.pem=./server-key.pem --from-file=cert.pem=./server-cert.pem  